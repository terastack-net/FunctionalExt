...
namespace CSharpFunctionalExtensions.Json.Serialization
{
    ...
    {
        public static ResultDto Success() => new(true, (Exception)null);
        public static ResultDto Failure(Exception error) => new(false, error);

        public static ResultDto<TValue> Success<TValue>(TValue value) => new(true, value, null);
        public static ResultDto<TValue> Failure<TValue>(Exception error) => new(false, default!, error);
    ...
    internal partial record ResultDto(bool IsSuccess, Exception? Error);

    internal record ResultDto<TValue>(bool IsSuccess, TValue Value, Exception? Error)
        : ResultDto<TValue, Exception>(IsSuccess, Value, Error);
