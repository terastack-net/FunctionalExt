...
{
    public class TapTests_Task_Left : TapTestsBase
    {
        [Theory]
        [InlineData(true)]
        [InlineData(false)]
        public async Task Tap_Task_Left_executes_action_on_result_success_and_returns_self(bool isSuccess)
        {
            Return result = Return.SuccessIf(isSuccess, ErrorMessage);

            var returned = await result.AsTask().Tap(Action);
        ...
        public async Task Tap_Task_Left_T_executes_action_T_on_result_success_and_returns_self(bool isSuccess)
        {
            ...
            var returned = await result.AsTask().Tap(Action_T);
        ...
        [Fact]
        public async Task Tap_Task_Left_UnitResult_E_executes_action_on_success_and_returns_self()
        {
        ...
        [Fact]
        public async Task Tap_Task_Left_UnitResult_E_executes_action_on_failure_and_returns_self()
        {
