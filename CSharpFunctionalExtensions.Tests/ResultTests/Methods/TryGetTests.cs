...
namespace CSharpFunctionalExtensions.Tests.ResultTests.Methods
{
    public class TryGetTests
    {
        ...
        {
            Return result = Return.Success(ErrorMessage);
        ...
        {
            Return<string> result = Return.Success("Success");
            result.TryGetError(out string error).Should().BeFalse();
            error.Should().BeNull();
        }
        ...
        {
            Return<string> result = Return.Failure<string>(ErrorMessage);
            result.TryGetError(out string error).Should().BeTrue();
            error.Should().Be(ErrorMessage);
        }
        ...
        {
            Return<string, string> result = Return.Success<string, string>("Success");
            result.TryGetError(out string error).Should().BeFalse();
            error.Should().BeNull();
        }
        ...
        {
            Return<string, string> result = Return.Failure<string, string>(ErrorMessage);
            result.TryGetError(out string error).Should().BeTrue();
            error.Should().Be(ErrorMessage);
        }
